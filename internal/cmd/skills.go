...
var skillsInstallCmd = &cobra.Command{
	...
	Long: `Install skill files that help AI agents (Claude Code, etc.) use the intervals CLI effectively.

Skills are structured Markdown files that teach agents about available commands,
required flags, safety rules, and multi-step training workflows.`,
...
func runSkillsGet(cmd *cobra.Command, args []string) error {
	...
	if _, err := skillsFS.ReadDir(skillDir); err != nil {
		return &cliexit.ValidationError{Err: fmt.Errorf("unknown skill %q. Run `intervals skills list` to see available skills", name)}
	}
